...
int main(){
    ...
    Efile.open("energyHB.dat");
    Mfile.open("magnetHB.dat");
    ANIMfile.open("animationHB.dat");

    std::cout << "Potts model with heat bath algorithm" << std::endl;
    ...
    // Random state selection for heat bath
    std::discrete_distribution<state> rstate({});
    ...
    for (t = 0; t < MCMAX; t++){
        ...
        rstate = std::discrete_distribution<state>(w.begin(), w.end());
        s_ = rstate(mt);
        lattice[p] = s_;
